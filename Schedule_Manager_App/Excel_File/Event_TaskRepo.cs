...
class Event_TaskRepo
{
    protected const string ExcelFilePath = "Event_TaskRepo.xlsx";
   
    private const string WorksheetName = "Task";

    // Serialize danh sách EvenTasks thành JSON byte array
    private static byte[] SerializeListTasks(List<EventTask> eventtasks)
    {
        return JsonSerializer.SerializeToUtf8Bytes(eventtasks);
    }

    // Serialize một EvenTasks thành JSON byte array
    private static byte[] SerializeTask(EventTask eventtask)
    {
        List<EventTask> tempList = new List<EventTask> { eventtask };
        return JsonSerializer.SerializeToUtf8Bytes(tempList);
    }

    // Deserialize JSON byte array thành danh sách Task
    private static List<EventTask> DeserializeTasks(byte[] data)
    {
        if (data == null || data.Length == 0)
        {
            return new List<EventTask>(); // Trả về danh sách rỗng nếu dữ liệu đầu vào không hợp lệ
        }

        List<EventTask>? tasks = JsonSerializer.Deserialize<List<EventTask>>(data);

        if (tasks == null)
        {
            return new List<EventTask>(); // Trả về danh sách rỗng nếu quá trình giải tuần tự hóa thất bại
        }
    ...
    public void SaveToExcel(List<EventTask> newTasks)
    {
    ...
    public void SaveTaskToExcel(EventTask newTask)
    {
    ...
    {
        if (!File.Exists(ExcelFilePath)) return new List<EventTask>();
        ...
        try
        {
            ...
            List<EventTask> allTasks = new List<EventTask>();
            ...
            foreach (IXLRow row in worksheet.RowsUsed())
            {
                ...
                if (base64Data != "")
                {
                    ...
                    List<EventTask> deserializedTasks = DeserializeTasks(serializedData);
    ...
    {
        List<EventTask> tasks = GetAllTasks();
        List<EventTask> newList = new List<EventTask>();
    ...
    // Cập nhật Task
    public void UpdateTask(EventTask updateTask)
    {
    ...
    // Tìm task theo Id
    public EventTask FindTaskById(int id)
    {
        List<EventTask> tasks = GetAllTasks();

        List<EventTask> temp = new List<EventTask>();
    ...
    public EventTask FindTaskByTaskname(string taskname)
    {
        ...
        // Lấy danh sách tất cả các Event task
        List<EventTask> tasks = GetAllTasks();
    ...
    // In danh sách Task
    public void PrintTasks(List<EventTask> tasks)
    {
    ...
    private void RewriteExcel(List<EventTask> tasks)
    {
